...
- **PNG** plot artifacts on disk (paths referenced from SQLite; never blob images into the DB).
- **Cache invalidation:** a **SHA-256** content hash over (raw input bytes + analysis parameters:
...
- Keyed recall of common quantities ("peak stress vs cycle for test A", "Coffin-Manson params").
