...
## Decision
- Depend on the **official `mcp` SDK**, pinned **`>=1.27,<2`**. This avoids the in-flux v2 alpha.
  Current installed version: 1.28.0.
...
- Tools are narrow and clearly named. Inputs validated with pydantic, and errors returned as clear
  text. Tested locally with the MCP Inspector (`mcp dev`).

## Consequences
- Stable build target now, with a clean upgrade path when v2 stabilizes (revisit in a new ADR).
