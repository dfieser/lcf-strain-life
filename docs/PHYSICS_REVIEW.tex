...
\subsection{Energy-based life, Halford-Morrow}
The stabilized plastic strain energy per cycle correlates with life through
the power law
\begin{equation}
\Delta W_p = W'_f \left(2N_f\right)^{\beta},
\end{equation}
with the energy-life coefficient \(W'_f\) in MJ/m\(^3\) and the exponent
\(\beta\) negative, fitted by log-log least squares like the strain-life
branches. For a material with Masing behavior, the plastic energy per cycle
follows from the loop parameters in closed form,
\begin{equation}
...
\(\Delta\sigma/2 = K'\left(\Delta\varepsilon_p/2\right)^{n'}\). The closed
form is exact for Masing behavior and an estimate otherwise, and the Masing
consistency check of the strain-life fit reports the departure. The energy
criterion is useful at elevated temperature, where strength drops while
ductility rises and a strain-only correlation misses the stress effect.
References: Morrow, ASTM STP 378, 1965, pp.\ 45--87; Halford, J.\ Materials
1 (1966) 3--18; applied to high-temperature low cycle fatigue by Zhang, Zuo,
and Liu, Fatigue Fract.\ Eng.\ Mater.\ Struct.\ 36 (2013) 623--630, Eqs.\ 2
and 3. Implementation validation: verified against the defining equations,
including the exact Masing-loop identity, not yet against an independent
golden dataset.
...
= \sum_{\text{failures}} \Big[\ln\phi(z_i) - \ln\sigma\Big]
+ \sum_{\text{runouts}} \ln S(z_i),
\end{equation}
...
\begin{equation}
q_L = \min\Big\{q : 2\big[\ell(\hat\theta) - \ell_p(q)\big]
\le z_\gamma^2\Big\},
\end{equation}
