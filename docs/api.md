---
description: >-
  API reference for the lcf Python package, generated from the docstrings.
---

# API reference

Generated from the docstrings of the public modules. The package facade
`import lcf` re-exports the everyday names, the module pages below are the
authoritative surface. Unit conventions everywhere: true stress in MPa,
strain as a dimensionless fraction, life in reversals, exponents b and c
negative.

## Ingestion and reduction

::: lcf.ingest

::: lcf.cycles

::: lcf.metrics

## Fitting and life

::: lcf.fits

::: lcf.life

::: lcf.meanstress

::: lcf.estimate

## Variable amplitude and damage

::: lcf.counting

::: lcf.damage

::: lcf.spectrum

::: lcf.simulate

## Notch

::: lcf.notch

## Statistics

::: lcf.stats

::: lcf.staircase

::: lcf.rfl

## Elevated temperature and evolution

::: lcf.hightemp

::: lcf.cyclic_evolution

## Multiaxial

::: lcf.multiaxial

::: lcf.criticalplane

## Data, interchange, and provenance

::: lcf.datasets

::: lcf.interchange

::: lcf.citations
